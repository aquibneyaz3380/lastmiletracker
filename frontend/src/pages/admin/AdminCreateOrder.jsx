// src/pages/admin/AdminCreateOrder.jsx
...
const AdminCreateOrder = () => {
  ...
  useEffect(() => {
    api.get('/admin/customers').then(res => setCustomers(res.data)).catch(console.error);
  }, []);
  ...
  return (
    <div className="max-w-2xl mx-auto mt-8 px-4 pb-12">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Create Order (Admin)</h2>
      ...
      <form onSubmit={handleSubmit} className="bg-white border rounded-lg p-6 shadow space-y-5">
        <div>
          ...
          <select name="customerId" required value={form.customerId} onChange={handleChange}
        ...
        <div className="grid grid-cols-4 gap-3">
          {['length','breadth','height'].map(f => (
            <div key={f}>
              <label className="block text-sm font-medium text-gray-700 mb-1 capitalize">{f} (cm)</label>
              <input type="number" step="0.1" name={f} required value={form[f]} onChange={handleChange}
                className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          ))}
          <div>
        ...
        {quote && (
          <div className="bg-brand-50 border border-brand-100 rounded-lg p-4 text-sm space-y-1">
            ...
            {quote.codSurcharge > 0 && <p><span className="text-gray-600">COD Surcharge:</span> <strong>₹{quote.codSurcharge.toFixed(2)}</strong></p>}
            <p className="text-base pt-2 border-t border-brand-200">
              <span className="text-gray-700">Total:</span> <strong className="text-brand-700">₹{quote.totalCharge.toFixed(2)}</strong>
            </p>
          </div>
        )}
